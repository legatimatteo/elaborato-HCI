...
\begin{figure}[H]
	...
	\label{fig:difficolta-c3-generale}
\end{figure}
...
I risultati quantitativi trovano riscontro nelle osservazioni qualitative e nei commenti raccolti tramite il protocollo \textit{thinking aloud}. Per la versione originale (A), sono emerse criticità ricorrenti:
...
Analizzando i dati in base all'ordine di esecuzione (Tabella~\ref{tab:6-c3-2}), emergono dinamiche coerenti con l’influenza predominante della struttura dell’interfaccia rispetto all’esperienza dell’utente.

Nel gruppo A--B, il tempo mediano di esecuzione passa da 1:53 (versione A) a 1:27 (versione B), con una riduzione del 23,01\%.

Nel gruppo B--A, gli utenti completano inizialmente il compito sulla versione B in 1:46, mentre nella successiva interazione con la versione A il tempo mediano aumenta fino a 2:02. Questo risultato suggerisce che l’esperienza maturata con la versione più ottimizzata non si trasferisce in modo efficace alla versione originale, le cui criticità strutturali continuano a incidere in maniera significativa sulle prestazioni, indipendentemente dalla familiarità con il task.
...
\begin{figure}[H]
	...
	\label{fig:difficolta-c3-ordine-confronto}
\end{figure}
...
\begin{table}[H]
	...
	\begin{tabular}{|>{\raggedright\arraybackslash}p{3cm}|>{\centering\arraybackslash}p{3.8cm}|>{\centering\arraybackslash}p{3.8cm}|>{\centering\arraybackslash}p{2.2cm}|}
		...
		Ordine B--A                   & 2:02                                       & 1:46                                       & -12,70\%            \\ \hline
	\end{tabular}
\end{table}
