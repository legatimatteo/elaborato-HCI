...
\begin{table}[H]
	...
	\begin{tabular}{|>{\raggedright\arraybackslash}p{2.2cm}|>{\centering\arraybackslash}p{1.0cm}|>{\centering\arraybackslash}p{1.0cm}|>{\centering\arraybackslash}p{1.8cm}|>{\centering\arraybackslash}p{1.6cm}|>{\centering\arraybackslash}p{1.0cm}|>{\centering\arraybackslash}p{1.0cm}|>{\centering\arraybackslash}p{1.8cm}|>{\centering\arraybackslash}p{1.6cm}|}
		...
		\textit{Errori (Media)}       & 1,22                                      & 0,667                                     & \textbf{-45,45\%}      & \pvalue{0,21446}   & 0,000      & 0,50       & \textbf{Non calcolabile} & \pvalue{0,20311}   \\ \hline
...
Questo divario si riflette coerentemente sulle valutazioni di difficoltà percepita (Figura~\ref{fig:6-c3-2}). Nel gruppo B--A si nota lo stacco più netto: la mediana della difficoltà sale da 2 a 3,5 nel passaggio alla vecchia interfaccia (A), a indicare che dopo aver provato il sistema ottimizzato, le barriere all'usabilità di quello originale (quali la nomenclatura poco intuitiva della sezione esami) risultano ancor più evidenti e penalizzanti. Questo sensibile peggioramento nella percezione soggettiva della difficoltà trova un riscontro oggettivo inequivocabile nei dati di interazione (Tabella \ref{tab:6-c3-2}). Nel passaggio dalla Versione B alla Versione A, infatti, il numero medio di click subisce un incremento drastico del +111,39\% (passando da 13,17 a 27,83). Il fatto che gli utenti siano stati costretti a raddoppiare letteralmente il numero di interazioni fisiche per completare lo stesso compito giustifica pienamente il picco di frustrazione e l'aumento del carico cognitivo dichiarati nei questionari. Nel gruppo A--B si osserva un percorso inverso, con la mediana che scende da 2 a 1. In questo caso, l'esperienza iniziale con la versione complessa ha agito da training, rendendo l'approccio alla versione B estremamente fluido, pur con piccoli margini di miglioramento legati ad elementi secondari.
