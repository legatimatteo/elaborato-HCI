...
\label{subsec:questionario_SUS}

In questa sezione vengono analizzati nel dettaglio i risultati ottenuti tramite il questionario System Usability Scale (SUS), somministrato per raccogliere dati quantitativi sulla percezione dell'usabilità complessiva del sistema. L'analisi confronta la versione originale (A) con la versione post-riprogettazione (B), valutando sia le metriche aggregate sia le dinamiche legate all'ordine di esecuzione dei test (A-B e B-A). Per convalidare la robustezza delle differenze osservate tra le medie dei punteggi, è stato condotto un T-test di Student per campioni appaiati a due code, impostando un livello di significatività convenzionale $\alpha = 0{,}05$.

Il calcolo del punteggio SUS segue un algoritmo standardizzato basato sulle risposte fornite dagli utenti alle 10 domande del questionario, ciascuno valutato su una scala Likert a 5 punti (da 1: ``Fortemente in disaccordo'' a 5: ``Fortemente d'accordo''). Le domande a indice dispari presentano una formulazione positiva, mentre quelli a indice pari hanno una formulazione negativa. Il punteggio finale, espresso su una scala da 0 a 100, viene calcolato come segue:
\begin{itemize}
	\item Per ciascuna risposta dispari ($i \in \{1, 3, 5, 7, 9\}$), si calcola il punteggio parziale sottraendo 1 alla risposta fornita dall'utente ($x_i - 1$);
	\item Per ciascuna risposta pari ($i \in \{2, 4, 6, 8, 10\}$), si calcola il punteggio parziale sottraendo il valore della risposta fornita a 5 ($5 - x_i$).
\end{itemize}
La somma di tutti i punteggi parziali dei 10 quesiti viene successivamente moltiplicata per $2{,}5$:
\begin{equation}
...
L'analisi aggregata dei punteggi SUS (rappresentata in Figura~\ref{fig:6-sus-1}) rivela una netta preferenza per la Versione B, che ottiene una media di 81,33. Questo valore certifica un'usabilità "eccellente", posizionandosi ben al di sopra della media standard di riferimento, fissata convenzionalmente a 68. Al contrario, la Versione A registra una media globale di 47,67, posizionandosi nettamente sotto la soglia di accettabilità. Il confronto tra le due medie rivela una differenza statisticamente altamente significativa ($p < 0{,}001$), confermando che il divario prestazionale non è frutto di fluttuazioni casuali.
...
\begin{figure}[H]
	...
	\begin{tikzpicture}
		...
		\end{axis}
	\end{tikzpicture}
	\caption{Confronto generale dei punteggi medi SUS tra Versione A e Versione B ($p < 0{,}001$).}
	\label{fig:6-sus-1}
\end{figure}
...
\begin{figure}[H]
	...
	\begin{tikzpicture}
		...
		\end{axis}
	\end{tikzpicture}
	\caption{Punteggi medi SUS relativi al sottogruppo con ordine di esecuzione B-A ($p = 0{,}001$).}
	\label{fig:6-sus-3}
\end{figure}
