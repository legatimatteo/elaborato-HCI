...
In primo luogo, abbiamo progettato l’esperienza, descrivendo in modo dettagliato l’oggetto di studio, ovvero BrainBox, il sistema che intendiamo analizzare. In questa fase abbiamo pianificato l’organizzazione temporale delle attività, definendo le scadenze e la distribuzione dei compiti all’interno del gruppo.

Successivamente, abbiamo definito le condizioni sperimentali, individuando come profilo utente di riferimento lo studente, in quanto principale destinatario e utilizzatore del sistema.
...
Di seguito sono riportati i dieci principi euristici di Nielsen (rielaborati da P.Mussio):
...
Al termine delle valutazioni individuali, sono stati unificati i problemi simili ed eliminati gli eventuali duplicati, è stato assegnato un grado di severità a ciascun problema e sono stati confermati i principi violati. Infine, sono stati elaborati i grafici di classificazione dei problemi presentati nelle sezioni successive di questo capitolo.
...
\input{valutazione-euristica/problemi-usabilita-f1}
\input{valutazione-euristica/problemi-usabilita-f2}
\input{valutazione-euristica/problemi-usabilita-f3}
\input{valutazione-euristica/problemi-usabilita-f4}

\subsubsection{Classificazione dei problemi individuati}
\input{valutazione-euristica/frequenza-gravità}

\subsubsection{Distribuzione dei problemi per numero di valutatori}
\input{valutazione-euristica/distribuzione-problemi}

\subsubsection{Confronto con la curva di Nielsen}
\input{valutazione-euristica/curva-nielsen}

\subsubsection{Frequenza delle violazioni}
\input{valutazione-euristica/frequenza-violazioni}