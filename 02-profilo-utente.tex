...
\subsection{Studente universitario}
A differenza di altri sistemi, \textit{BrainBox} è pensata per una sola categoria principale: lo \textbf{Studente universitario}. Nella piattaforma, infatti, lo studente è colui che utilizza la piattaforma per organizzare e condividere i propri appunti con la \textit{community}. Ogni studente può caricare gli appunti rendendoli privati o pubblici, consentendo ad altri studenti di leggerli, scaricarli e recensirli.

Per questo motivo, nelle tabelle seguenti non è stato fatto un confronto tra profili diversi, ma si è preferito definire le caratteristiche dello \textit{studente medio} a cui si rivolge l'applicazione. È stato comunque tenuto in considerazione che all'interno di questa categoria siano presenti persone molto diverse tra loro (per tempo a disposizione, abitudini di studio, capacità informatiche o eventuali disabilità). Pertanto, invece di appesantire questo profilo generale, si è deciso di rappresentare e analizzare tali differenze nella sezione successiva \ref{subsec:personas}, attraverso la creazione di 5 diverse \textit{Personas}.
...
Abbiamo ipotizzato una motivazione nell'utilizzo dell'applicazione alta e al contempo un'attitudine positiva, in quanto la condivisione del materiale didattico con la \textit{community} può semplificare di molto il suo periodo di studio.
...
Lo studente inizialmente può avere un'esperienza media con l'applicazione, poiché si tratta di un nuovo strumento da esplorare. Tuttavia, grazie all'interfaccia intuitiva e alle analogie con altre piattaforme di \textit{note-taking} o studio (come Google Docs o OneNote), l'apprendimento all'uso risulta rapido.

Lo studente universitario medio possiede un'alta competenza informatica di base: utilizza regolarmente PC, browser, piattaforme di \textit{e-learning} (come Moodle o Teams) e strumenti di collaborazione online.
...
Statisticamente, una percentuale significativa della popolazione presenta daltonismo (circa il 9\% degli uomini e lo 0,5\% delle donne). È quindi importante che l'interfaccia rispetti i principi di accessibilità visiva, garantendo un adeguato contrasto cromatico e prevedendo alternative grafiche, come icone o etichette testuali, così da offrire un'esperienza d'uso efficace anche agli utenti con lievi deficit visivi.

Il profilo utente di riferimento non presenta disabilità fisiche o cognitive che influenzano l'uso del sistema. L'applicazione è pensata per studenti senza limitazioni motorie o sensoriali significative.
...
L'applicazione è progettata per essere intuitiva e di facile apprendimento, quindi non richiede un addestramento formale essendo \textit{walk-up-and-use}. Lo studente può acquisire rapidamente familiarità con le funzionalità principali attraverso l'esplorazione diretta.
