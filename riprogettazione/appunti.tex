...
\begin{scenario}{Scenario: Primo accesso alla pagina I Miei Appunti}
  ...
  Procedo comunque a creare la mia prima cartella. Il sistema mi chiede il nome e se voglio renderla pubblica tramite un flag; clicco su \textit{«Crea cartella»} e vedo che l'operazione va a buon fine. Entro nella cartella appena creata, ma mi ritrovo punto e a capo: compare di nuovo il messaggio che non sono stati trovati documenti e, per creare una sotto-cartella, devo di nuovo andare a cercare il tasto in alto nella navbar. Una volta creata anche la sotto-cartella, \textbf{cerco un tasto \textit{«Indietro»} per risalire il percorso, ma non lo trovo}. Mi accorgo
  ...
  correttamente tra loro}. Dopo vari tentativi, se ho la fortuna di trovare una combinazione che il sistema accetta, arrivo finalmente allo step \textit{«Fatto»}.
...
Da questo scenario sorgono molti problemi di usabilità. In particolare, viene sottolineata la presenza di molto testo in inglese, non coerente con l'interfaccia in italiano, la posizione dei bottoni per caricare i file e creare nuove cartelle poco intuitiva e la non possibilità di controllare i dati inseriti prima del caricamento di un file.
...
\begin{itemize}
  ...
  Problemi di usabilità risolti:
    \begin{itemize}
      \item P27 (Il titolo dello step di caricamento "Upload" è in inglese): E' stato tradotto lo step "Upload" in "Allegato".
      \item P28 (Il titolo "Fatto" è generico e non consente di rivedere i dati inseriti prima del salvataggio): E' stato tradotto lo step "Fatto" in "Riepilogo". 
    \end{itemize} 
  ...
  Problemi di usabilità risolti:
    \begin{itemize}
      \item P59 (Nessun messaggio di errore se si carica un file non supportato): Sono stati aggiunti messaggi che informano l'utente sui vincoli di caricamento accettati dal sistema.
    \end{itemize} 
  ...
  Problemi di usabilità risolti:
  \begin{itemize}
    \item P24 (Il tab "I Miei Materiali" ha un titolo fuorviante): Il tab sono stati rinominati con "I Miei Appunti".
    \item P25 (Una volta selezionato un tab non è più possibile navigare tra le sezioni perché la barra dei tab scompare): I tab sono ora sempre visibili cliccabili indipendentemente dalla sezione aperta.
