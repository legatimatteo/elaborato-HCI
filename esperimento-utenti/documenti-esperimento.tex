...
\begin{table}[H]
  ...
  \begin{tabularx}{\textwidth}{|>{\raggedright\bfseries\arraybackslash}p{3.5cm}|>{\raggedright\arraybackslash}X|}
    ...
    \begin{itemize}[itemsep=2pt, topsep=2pt, parsep=0pt]
        ...
        \item caricare i file necessari su \textit{BrainBox}
        \item attivare il tool per il conteggio del numero di click
        \item nel caso in cui si usassero i seed, eseguire il comando \texttt{php artisan migrate:fresh --seed} (solo la prima volta)
        \item Per versione A: eseguire i comandi \texttt{npm run dev} e \texttt{php artisan serve}
        \item Per versione B: eseguire il comando \texttt{dev.bat} su windows e \texttt{./dev.sh} su linux
    \end{itemize}
    \vspace{-6pt} \\
    \hline
    ...
    \begin{itemize}[itemsep=2pt, topsep=2pt, parsep=0pt]
        ...
        \item sottolineare che l'esperimento è volto alla valutazione del sito non dell'utente
        ...
        \item fare firmare il consenso informato
    ...
    \begin{itemize}[itemsep=2pt, topsep=2pt, parsep=0pt]
        ...
        \item se l'utente chiede aiuto, spiegare cortesemente che non è possibile fornire consigli, salvo casi estremi
        \item rispettare l'utente
        \item prevenire interruzioni esterne
    \end{itemize}
    \vspace{-6pt} \\
    \hline
    Dopo l'esperimento &
